...
thrust::host_vector<thrust::host_vector<cufftComplex>> F_grid_maker(const int& grid_dimX, const int& grid_dimY, const int& grid_dimZ, const float& deltaX, const float& deltaY, const float& deltaZ) {
    ...
    thrust::host_vector<thrust::host_vector<cufftComplex>> flat_host_grid(6, thrust::host_vector<cufftComplex>(ext_dimZ * ext_dimX * ext_dimY));
            ...
            for (int i = -grid_dimX + 1; i < grid_dimX; i++) {
                for (int j = -grid_dimY + 1; j < grid_dimY; j++) {
                    ...
                    case 0: // Nxx
                        ...
                        flat_host_grid[cell][kIdx*ext_dimX*ext_dimY+ iIdx*ext_dimY + jIdx].x = Nxx_calc(x, y, z, deltaX, deltaY, deltaZ, &f);
                        break;
                    case 1: // Nyx
                        //host_grid[cell][kIdx][iIdx][jIdx].x = Nxx_calc(x, y, z, deltaX, deltaY, deltaZ, &g);
                        flat_host_grid[cell][kIdx * ext_dimX * ext_dimY + iIdx * ext_dimY + jIdx].x = Nxx_calc(x, y, z, deltaX, deltaY, deltaZ, &g);
                        break;
                    case 2: // Nyz
                        //host_grid[cell][kIdx][iIdx][jIdx].x = Nxx_calc(x, z, y, deltaX, deltaZ, deltaY, &g);
                        flat_host_grid[cell][kIdx * ext_dimX * ext_dimY + iIdx * ext_dimY + jIdx].x = Nxx_calc(x, z, y, deltaX, deltaZ, deltaY, &g);
                        break;
                    case 3: // Nyy
                        //host_grid[cell][kIdx][iIdx][jIdx].x = Nxx_calc(y, z, x, deltaY, deltaZ, deltaX, &f);
                        flat_host_grid[cell][kIdx * ext_dimX * ext_dimY + iIdx * ext_dimY + jIdx].x = Nxx_calc(y, z, x, deltaY, deltaZ, deltaX, &f);
                        break;
                    case 4: // Nyz
                        //host_grid[cell][kIdx][iIdx][jIdx].x = Nxx_calc(y, z, x, deltaY, deltaZ, deltaX, &g);
                        flat_host_grid[cell][kIdx * ext_dimX * ext_dimY + iIdx * ext_dimY + jIdx].x = Nxx_calc(y, z, x, deltaY, deltaZ, deltaX, &g);
                        break;
                    case 5: // Nzz
                        //host_grid[cell][kIdx][iIdx][jIdx].x = Nxx_calc(z, x, y, deltaZ, deltaX, deltaY, &f);
                        flat_host_grid[cell][kIdx * ext_dimX * ext_dimY + iIdx * ext_dimY + jIdx].x = Nxx_calc(z, x, y, deltaZ, deltaX, deltaY, &f);
                        break;
                    default:
                        break;
                    }
    ...
    thrust::host_vector<thrust::host_vector<cufftComplex>> F_host_grid(6, thrust::host_vector<cufftComplex>(ext_dimZ*ext_dimX*ext_dimY));
    ...
    for (int cell = 0; cell < CELLSIZE; cell++) {
        ...
        //            device_grid[k * ext_dimX * ext_dimY + i * ext_dimY + j] = host_grid[cell][k][i][j];    //version for 3D
        //        }
        //    }
        //}

        thrust::copy(flat_host_grid[cell].begin(), flat_host_grid[cell].end(), device_grid.begin());
        ...
        thrust::copy(device_grid.begin(), device_grid.end(), F_host_grid[cell].begin());
    }

    cufftDestroy(plan);
    device_grid.clear();
    flat_host_grid.clear();
...
void TEST_F_grid_maker() {
    ...
    thrust::host_vector<thrust::host_vector<cufftComplex>> F_grid = F_grid_maker(grid_dimX, grid_dimY, grid_dimZ, deltaX, deltaY, deltaZ);
    flat_host_printer_cufft(F_grid[0], 2 * grid_dimZ, 2 * grid_dimX, 2 * grid_dimY);
}
...
int main() {
    ...
    //cpu
    thrust::host_vector<float> m_grid(2 * grid_dimZ * 2 * grid_dimX * 2 * grid_dimY);
    thrust::host_vector<float> exch_field(grid_dimZ * grid_dimX * grid_dimY);

    float exch_coeff = 1;

    for (int k = 0; k < grid_dimZ; k++) {
        for (int i = 0; i < grid_dimX; i++) {
            for (int j = 0; j < grid_dimY; j++) {



                if (i == 0 || i == grid_dimX - 1) {
                    exch_field[k * grid_dimX * grid_dimY + i * grid_dimY + j] += (i == 0) ? m_grid[k * grid_dimX * grid_dimY + (i + 1) * grid_dimY + j] - m_grid[k * grid_dimX * grid_dimY + (i) * grid_dimY + j]:
                                                                                            m_grid[k * grid_dimX * grid_dimY + (i - 1) * grid_dimY + j] - m_grid[k * grid_dimX * grid_dimY + (i) * grid_dimY + j];
                }
                else {
                    exch_field[k * grid_dimX * grid_dimY + i * grid_dimY + j] += m_grid[k * grid_dimX * grid_dimY + (i + 1) * grid_dimY + j] + m_grid[k * grid_dimX * grid_dimY + (i - 1) * grid_dimY + j]
                                                                               - 2 * m_grid[k * grid_dimX * grid_dimY + (i)*grid_dimY + j];
                }

                if (j == 0 || j == grid_dimY - 1) {
                    exch_field[k * grid_dimX * grid_dimY + i * grid_dimY + j] += (j == 0) ? m_grid[k * grid_dimX * grid_dimY + i * grid_dimY + (j+1)] - m_grid[k * grid_dimX * grid_dimY + i*grid_dimY + j] :
                                                                                            m_grid[k * grid_dimX * grid_dimY + i * grid_dimY + (j-1)] - m_grid[k * grid_dimX * grid_dimY + i*grid_dimY + j];
                }
                else {
                    exch_field[k * grid_dimX * grid_dimY + i * grid_dimY + j] += m_grid[k * grid_dimX * grid_dimY + i * grid_dimY + (j+1)] + m_grid[k * grid_dimX * grid_dimY + i * grid_dimY + (j-1)]
                                                                               - 2 * m_grid[k * grid_dimX * grid_dimY + i*grid_dimY + (j)];
                }

                if (k == 0 || k == grid_dimZ - 1) {
                    exch_field[k * grid_dimX * grid_dimY + i * grid_dimY + j] += (k == 0) ? m_grid[(k+1) * grid_dimX * grid_dimY + i * grid_dimY + j] - m_grid[k * grid_dimX * grid_dimY + i * grid_dimY + j] :
                                                                                            m_grid[(k-1) * grid_dimX * grid_dimY + i * grid_dimY + j] - m_grid[k * grid_dimX * grid_dimY + i * grid_dimY + j];
                }
                else {
                    exch_field[k * grid_dimX * grid_dimY + i * grid_dimY + j] += m_grid[(k+1) * grid_dimX * grid_dimY + i * grid_dimY + j] + m_grid[(k-1) * grid_dimX * grid_dimY + i * grid_dimY + j]
                                                                               - 2 * m_grid[k * grid_dimX * grid_dimY + i * grid_dimY + (j)];
                }

                exch_field[k * grid_dimX * grid_dimY + i * grid_dimY + j] *= exch_coeff;
